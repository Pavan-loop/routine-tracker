...
export default function Dashboard({ user: _user, setView }: Props) {
  ...
  useEffect(() => {
    async function load() {
      const [items, logs, tasks, scans, workouts] = await Promise.all([
        api.diet.getItems().catch(() => []),
        api.diet.getLogs(today).catch(() => []),
        api.tasks.getByDate(today).catch(() => []),
        api.bodyScans.getAll().catch(() => []),
        api.workouts.getByDate(today).catch(() => []),
      ])

      setDietTotal(items.length)
      setDietDone(logs.length)
      setTaskTotal(tasks.length)
      setTaskDone(tasks.filter(t => t.completed).length)
      ...
      if (consumed.some(i => i.kcal != null)) {
        setDietMacros({
          ...
          protein: Math.round(consumed.reduce((s, i) => s + (i.protein ?? 0), 0) * 10) / 10,
          carbs: Math.round(consumed.reduce((s, i) => s + (i.carbs ?? 0), 0) * 10) / 10,
          fat: Math.round(consumed.reduce((s, i) => s + (i.fat ?? 0), 0) * 10) / 10,
        })
      }

      setWorkoutCalories(workouts.reduce((s, w) => s + (w.calories_burned ?? 0), 0))
  ...
  return (
    <div className="space-y-5">
      ...
      <DietCard done={dietDone} total={dietTotal} macros={dietMacros} onClick={() => setView('diet')} />

      <div className="grid grid-cols-2 gap-3">
        <WorkoutCard calories={workoutCalories} count={workoutCount} names={workoutNames} onClick={() => setView('workout')} />
        <SummaryCard title="Tasks" done={taskDone} total={taskTotal} color="indigo" emptyLabel="Add tasks" onClick={() => setView('tasks')} />
      </div>

      {latestScan ? (
        <button onClick={() => setView('body-scan')} className="w-full text-left bg-zinc-900 border border-zinc-800 rounded-xl p-4 hover:border-zinc-700 transition-colors">
        ...
        <button onClick={() => setView('body-scan')} className="w-full text-left bg-zinc-900 border border-dashed border-zinc-800 rounded-xl p-4 hover:border-zinc-700 transition-colors">
...
function DietCard({ done, total, macros, onClick }: { done: number; total: number; macros: DietMacros | null; onClick: () => void }) {
...
function WorkoutCard({ calories, count, names, onClick }: { calories: number; count: number; names: string[]; onClick: () => void }) {
  return (
    <button onClick={onClick} className="bg-zinc-900 border border-zinc-800 rounded-xl p-4 text-left hover:border-zinc-700 transition-colors w-full">
      ...
      {count === 0 ? (
        ...
        <div>
          ...
          {names.length > 0 && <p className="text-[10px] text-zinc-700 mt-1.5 truncate">{names.slice(0, 3).join(' · ')}</p>}
        </div>
      )}
    </button>
  )
...
function SummaryCard({ title, done, total, color, emptyLabel, onClick }: { title: string; done: number; total: number; color: 'emerald' | 'indigo'; emptyLabel: string; onClick: () => void }) {
  const pct = total > 0 ? Math.round((done / total) * 100) : 0
  ...
  return (
    <button onClick={onClick} className="bg-zinc-900 border border-zinc-800 rounded-xl p-4 text-left hover:border-zinc-700 transition-colors">
...
const GOALS = [
  { key: 'lose', label: 'Lose Fat', offset: -400, color: 'text-sky-400', activeBg: 'bg-sky-500/10 border-sky-500/30 text-sky-400', techNote: (tdee: number) => `${tdee - 400} kcal/day — 400 kcal below maintenance`, simpleNote: 'Eat less than you burn. Your body digs into stored fat for the missing energy.' },
  { key: 'recomp', label: 'Recomp', offset: -300, color: 'text-orange-400', activeBg: 'bg-orange-500/10 border-orange-500/30 text-orange-400', techNote: (tdee: number) => `${tdee - 300} kcal/day — 300 kcal deficit with high protein`, simpleNote: 'Lose fat AND build muscle at the same time. The scale barely moves but your body shape changes.' },
  { key: 'maintain', label: 'Maintain', offset: 0, color: 'text-amber-400', activeBg: 'bg-amber-500/10 border-amber-500/30 text-amber-400', techNote: (tdee: number) => `${tdee} kcal/day — energy in = energy out`, simpleNote: 'Eat exactly what you burn. Your weight stays the same.' },
  { key: 'build', label: 'Build Muscle', offset: 250, color: 'text-emerald-400', activeBg: 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400', techNote: (tdee: number) => `${tdee + 250} kcal/day — 250 kcal surplus above maintenance`, simpleNote: 'Eat a little more than you burn. Extra fuel helps your muscles grow.' },
] as const
...
function CaloriesCard({ scan, prevScan }: { scan: BodyScan; prevScan: BodyScan | null }) {
  ...
  if (prevScan && prevScan.weight != null && prevScan.fat != null) {
    ...
    if (weightStable && fatLost && muscleGained) trendNote = `Body recomposition — weight unchanged but fat is down and muscle is up. This is the best possible result. Keep doing exactly what you're doing.`
    else if (weightStable && fatGained && muscleLost) trendNote = `Weight looks the same but fat went up and muscle dropped. Your body composition is getting worse. Increase protein intake and add resistance training.`
    else if (weightStable) trendNote = `Weight is stable since last scan. No significant fat or muscle changes detected.`
    else if (weightLost && fatLost && muscleLost) trendNote = `Lost ${Math.abs(weightDiff).toFixed(1)}kg since last scan — fat is down but muscle dropped slightly too. Eat more protein to protect muscle while cutting.`
    else if (weightLost && fatLost && !muscleLost) trendNote = `Lost ${Math.abs(weightDiff).toFixed(1)}kg since last scan — fat is down, muscle is maintained. Good fat loss. Keep it up.`
    else if (weightLost && fatGained) trendNote = `Lost ${Math.abs(weightDiff).toFixed(1)}kg but fat % went up — you're losing muscle, not fat. Increase protein significantly and add strength training.`
    else if (weightGained && fatLost) trendNote = `Gained ${weightDiff.toFixed(1)}kg but fat % is down — you're adding muscle. Good lean bulk or recomposition progress.`
    else if (weightGained && fatGained && muscleLost) trendNote = `Gained ${weightDiff.toFixed(1)}kg — fat increased and muscle dropped. Consider reducing calories and adding resistance training.`
    else if (weightGained && fatGained && muscleGained) trendNote = `Gained ${weightDiff.toFixed(1)}kg — both muscle and fat went up. Normal for a bulk phase. Watch the fat % and consider a cut soon if it keeps rising.`
    else if (weightGained && fatGained) trendNote = `Gained ${weightDiff.toFixed(1)}kg — mostly fat gain. You're likely eating above maintenance. Try reducing by 300–400 kcal/day.`
    else if (weightGained && muscleGained) trendNote = `Gained ${weightDiff.toFixed(1)}kg — muscle is up, fat is stable. Clean lean bulk is working well.`
  }
  ...
  return (
    <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4 space-y-4">
      ...
      <div>
        <p className="text-xs text-zinc-500 mb-1">Maintenance — <span className="text-zinc-400">what you burn if weight stays the same</span></p>
      ...
      <div>
        ...
        <div className="flex gap-1.5">
          {ACTIVITY_LEVELS.map(level => (
            <button key={level.key} onClick={() => handleSetActivity(level.key)}
              className={`flex-1 py-1.5 px-1 rounded-lg text-xs font-medium transition-colors border ${
                activity === level.key ? 'bg-amber-500/10 border-amber-500/30 text-amber-400' : 'bg-zinc-800 border-transparent text-zinc-500 hover:text-zinc-300'
              }`}
            >
              {level.label}
            </button>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-1.5">
          {GOALS.map(g => (
            <button key={g.key} onClick={() => handleSetGoal(g.key)}
              className={`py-2 px-2 rounded-lg text-xs font-medium transition-colors border ${
                goal === g.key ? g.activeBg : 'bg-zinc-800 border-transparent text-zinc-500 hover:text-zinc-300'
              }`}
            >
              {g.label}
            </button>
      ...
      {goal === 'recomp' && <RecompPanel scan={scan} />}
      {goal === 'lose' && <LosePanel scan={scan} />}
      {goal === 'build' && <BuildPanel scan={scan} />}
...
function ProteinTargets({ scan, color }: { scan: BodyScan; color: string }) {
  return (
    <div>
      <p className={`text-xs ${color} font-medium mb-2`}>Daily protein target</p>
      <div className="grid grid-cols-2 gap-2">
        ...
        <div className="bg-zinc-900/60 rounded-lg p-2 text-center">
          <p className={`text-base font-bold ${color}`}>{Math.round(scan.weight! * 2.2)}g</p>
...
function RecompPanel({ scan }: { scan: BodyScan }) {
  return (
    <div className="bg-orange-500/5 border border-orange-500/15 rounded-xl p-3 space-y-3">
      <div className="flex gap-2">
        <span className="text-base leading-none mt-0.5">{scan.fat! >= 20 ? '🟢' : scan.fat! >= 15 ? '🟡' : '🔴'}</span>
        <div>
          <p className="text-xs font-medium text-orange-300">{scan.fat! >= 20 ? 'Great candidate for recomp' : scan.fat! >= 15 ? 'Good candidate for recomp' : 'Harder at low body fat'}</p>
          <p className="text-xs text-zinc-500 mt-0.5">{scan.fat! >= 20 ? `Higher fat % (${scan.fat}%) means your body has plenty of stored energy to fuel muscle growth.` : scan.fat! >= 15 ? `At ${scan.fat}% body fat you'll see solid recomp results with consistent training and high protein.` : `At ${scan.fat}% body fat, traditional cut/bulk cycles may produce faster results than recomp.`}</p>
        </div>
      </div>
      <ProteinTargets scan={scan} color="text-orange-400" />
      <div>
        <p className="text-xs text-orange-400 font-medium mb-1.5">Non-negotiable requirements</p>
        <div className="space-y-1">
          {[{ icon: '🏋️', text: 'Resistance training 3–5×/week (progressive overload)' }, { icon: '😴', text: '7–9 hours of sleep — HGH released during sleep builds muscle' }, { icon: '📅', text: 'Results visible in 8–12 weeks — the scale barely moves, track body fat % instead' }].map(r => (
            <div key={r.text} className="flex gap-2"><span className="text-xs leading-5">{r.icon}</span><p className="text-xs text-zinc-400">{r.text}</p></div>
          ))}
        </div>
      </div>
    </div>
...
function LosePanel({ scan }: { scan: BodyScan }) {
  return (
    <div className="bg-sky-500/5 border border-sky-500/15 rounded-xl p-3 space-y-3">
      <div className="flex gap-2">
        <span className="text-base leading-none mt-0.5">{scan.fat! >= 25 ? '🟢' : scan.fat! >= 15 ? '🟡' : '🔴'}</span>
        <div>
          <p className="text-xs font-medium text-sky-300">{scan.fat! >= 25 ? 'Great candidate for fat loss' : scan.fat! >= 15 ? 'Good candidate — protect your muscle' : 'Already lean — be careful'}</p>
          <p className="text-xs text-zinc-500 mt-0.5">{scan.fat! >= 25 ? `At ${scan.fat}% body fat you have plenty to lose. You can handle a 400–500 kcal deficit safely.` : scan.fat! >= 15 ? `At ${scan.fat}% body fat, use the higher end protein (2.2g/kg) — your body will try to burn muscle too.` : `At ${scan.fat}% you're already lean. Keep the deficit small (300 kcal max) and protein very high or you'll lose muscle.`}</p>
        </div>
      </div>
      <ProteinTargets scan={scan} color="text-sky-400" />
      <div>
        <p className="text-xs text-sky-400 font-medium mb-1.5">Key requirements</p>
        <div className="space-y-1">
          {[{ icon: '🏋️', text: 'Resistance training 3–4×/week — without this, you lose muscle AND fat' }, { icon: '🥩', text: 'Spread protein across 4–5 meals, 20–40g each sitting' }, { icon: '⚠️', text: 'Keep deficit at 300–500 kcal — bigger deficits burn muscle, not just fat' }].map(r => (
            <div key={r.text} className="flex gap-2"><span className="text-xs leading-5">{r.icon}</span><p className="text-xs text-zinc-400">{r.text}</p></div>
          ))}
        </div>
      </div>
...
function BuildPanel({ scan }: { scan: BodyScan }) {
  return (
    <div className="bg-emerald-500/5 border border-emerald-500/15 rounded-xl p-3 space-y-3">
      <div className="flex gap-2">
        <span className="text-base leading-none mt-0.5">{scan.fat! < 15 ? '🟢' : scan.fat! <= 20 ? '🟡' : '🔴'}</span>
        <div>
          <p className="text-xs font-medium text-emerald-300">{scan.fat! < 15 ? 'Ideal starting point for a bulk' : scan.fat! <= 20 ? 'Good — keep the surplus clean' : 'Consider recomp first'}</p>
          <p className="text-xs text-zinc-500 mt-0.5">{scan.fat! < 15 ? `At ${scan.fat}% body fat you can bulk longer before needing to cut. Great position to be in.` : scan.fat! <= 20 ? `At ${scan.fat}% body fat, stick to a clean surplus (300–400 kcal). Going higher will mostly add fat.` : `At ${scan.fat}% you'll gain a lot of fat in a surplus. Consider recomp or a cut first to get below 18%.`}</p>
        </div>
      </div>
      <ProteinTargets scan={scan} color="text-emerald-400" />
      <div>
        <p className="text-xs text-emerald-400 font-medium mb-1.5">Key requirements</p>
        <div className="space-y-1">
          {[{ icon: '🏋️', text: 'Progressive overload 3–4×/week — consistently lift heavier over time' }, { icon: '😴', text: '7–9 hours sleep — muscle is built during sleep, not in the gym' }, { icon: '📊', text: 'Aim for 0.25–0.5kg gain/week — faster than that is mostly fat' }].map(r => (
            <div key={r.text} className="flex gap-2"><span className="text-xs leading-5">{r.icon}</span><p className="text-xs text-zinc-400">{r.text}</p></div>
          ))}
        </div>
